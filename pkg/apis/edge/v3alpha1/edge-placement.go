...
type EdgePlacementSpec struct {
	...
	// `namespacedDownsync` selects the namespaced objects to bind with the selected Locations for downsync.
	// An object is selected if it matches at least one member of this list.
	// +optional
	NamespacedDownsync []DownsyncObjectTest `json:"namespacedDownsync,omitempty"`

	// `clusterScopedDownsync` selects the cluster scoped objects to bind with the selected Locations for downsync.
	// An object is selected if it matches at least one member of this list.
	// +optional
	ClusterScopedDownsync []DownsyncObjectTestSansNamespace `json:"clusterScopedDownsync,omitempty"`
...
// - the `DownsyncObjectTestSansNamespace` criterion is satisfied;
// - the `namespaceNames` criterion is satisfied; and
...
// Validation may be done by a controller that posts violations in the status section
// of the EdgePlacement.
type DownsyncObjectTest struct {
	DownsyncObjectTestSansNamespace `json:",inline"`

	// `namespaceNames` is a list of acceptable names for the object's namespace.
	// An entry of `"*"` means that any namespace is acceptable.
	// If this list contains `"*"` then it should contain nothing else.
	// The default value for this list is `["*"]`.
	// +optional
	NamespaceNames []string `json:"namespaceNames,omitempty"`

	// `namespaceSelectors` a list of label selectors.
	// At least one of them must match the labels of the Namespace object
	// that defines the namespace of the object that this DownsyncObjectTest is testing.
	// The default value of this list is `[{}]` (one selector that matches every set of labels).
	// +optional
	NamespaceSelectors []metav1.LabelSelector `json:"namespaceSelectors,omitempty"`
}

// DownsyncObjectTestSansNamespace is a set of criteria that characterize matching objects
// without regard to namespace.
// An object matches if:
// - the `apiGroup` criterion is satisfied;
// - the `resources` criterion is satisfied;
// - the `namespaceNames` criterion is satisfied;
// - the `namespaceSelectors` criterion is satisfied;
// - the `objectNames` criterion is satisfied; and
// - the `objectSelectors` criterion is satisfied.
// At least one of the fields must make some discrimination,
// when used to test cluster-scoped objects;
// it is not valid for every field to match all objects.
// Validation may be done by a controller that posts violations in the status section
// of the EdgePlacement.
type DownsyncObjectTestSansNamespace struct {
	// `apiGroup` is the API group of the referenced object, `"*` to match any API group.
	// The empty string identifies the Kubernetes core API group.
	...
	// `objectNames` is a list of acceptable names for the object.
	// An entry of `"*"` means that any object name is acceptable.
	// If this list contains `"*"` then it should contain nothing else.
	// The default value for this list is `["*"]`.
	// +optional
	...
	// The default value for this list is `[{}]` (one selector that matches every set of labels).
	// +optional
...
type EdgePlacementStatus struct {
	...
	// `validationErrorsFromController` holds error messages about invalid parts
	// of the spec. Those parts are simply ignored. This holds the complete lit of
	// messages from analyzing the spec version indicated by `specGeneration`.
	ValidationErrorsFromController []string `json:"validationErrorsFromController,omitempty"`
