...
type LocationClusterLister interface {
	...
	List(selector labels.Selector) (ret []*edgev2alpha1.Location, err error)
...
func (s *locationClusterLister) List(selector labels.Selector) (ret []*edgev2alpha1.Location, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*edgev2alpha1.Location))
	})
	return ret, err
}
...
type LocationLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*edgev2alpha1.Location, error)
...
func (s *locationLister) List(selector labels.Selector) (ret []*edgev2alpha1.Location, err error) {
	err = kcpcache.ListAllByCluster(s.indexer, s.clusterName, selector, func(i interface{}) {
		ret = append(ret, i.(*edgev2alpha1.Location))
	})
	return ret, err
}

// Get retrieves the Location from the indexer for a given workspace and name.
func (s *locationLister) Get(name string) (*edgev2alpha1.Location, error) {
	...
	if !exists {
		return nil, errors.NewNotFound(edgev2alpha1.Resource("Location"), name)
	}
	return obj.(*edgev2alpha1.Location), nil
}
...
// List lists all Locations in the indexer for a workspace.
func (s *locationScopedLister) List(selector labels.Selector) (ret []*edgev2alpha1.Location, err error) {
...
// Get retrieves the Location from the indexer for a given workspace and name.
func (s *locationScopedLister) Get(name string) (*edgev2alpha1.Location, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
