...
import (
	...
	v2alpha1 "k8s.io/sample-controller/pkg/apis/edge/v2alpha1"
)
...
type PlacementDecisionLister interface {
	...
	List(selector labels.Selector) (ret []*v2alpha1.PlacementDecision, err error)
	// Get retrieves the PlacementDecision from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2alpha1.PlacementDecision, error)
...
// List lists all PlacementDecisions in the indexer.
func (s *placementDecisionLister) List(selector labels.Selector) (ret []*v2alpha1.PlacementDecision, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2alpha1.PlacementDecision))
	})
	return ret, err
}

// Get retrieves the PlacementDecision from the index for a given name.
func (s *placementDecisionLister) Get(name string) (*v2alpha1.PlacementDecision, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2alpha1.Resource("placementdecision"), name)
	}
	return obj.(*v2alpha1.PlacementDecision), nil
}
