...
type SyncerConfigClusterLister interface {
	...
	List(selector labels.Selector) (ret []*edgev2alpha1.SyncerConfig, err error)
...
func (s *syncerConfigClusterLister) List(selector labels.Selector) (ret []*edgev2alpha1.SyncerConfig, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*edgev2alpha1.SyncerConfig))
	})
	return ret, err
}
...
type SyncerConfigLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*edgev2alpha1.SyncerConfig, error)
...
func (s *syncerConfigLister) List(selector labels.Selector) (ret []*edgev2alpha1.SyncerConfig, err error) {
	err = kcpcache.ListAllByCluster(s.indexer, s.clusterName, selector, func(i interface{}) {
		ret = append(ret, i.(*edgev2alpha1.SyncerConfig))
	})
	return ret, err
}

// Get retrieves the SyncerConfig from the indexer for a given workspace and name.
func (s *syncerConfigLister) Get(name string) (*edgev2alpha1.SyncerConfig, error) {
	...
	if !exists {
		return nil, errors.NewNotFound(edgev2alpha1.Resource("SyncerConfig"), name)
	}
	return obj.(*edgev2alpha1.SyncerConfig), nil
}
...
// List lists all SyncerConfigs in the indexer for a workspace.
func (s *syncerConfigScopedLister) List(selector labels.Selector) (ret []*edgev2alpha1.SyncerConfig, err error) {
...
// Get retrieves the SyncerConfig from the indexer for a given workspace and name.
func (s *syncerConfigScopedLister) Get(name string) (*edgev2alpha1.SyncerConfig, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
