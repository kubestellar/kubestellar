...
type SyncTargetClusterLister interface {
	...
	List(selector labels.Selector) (ret []*edgev2alpha1.SyncTarget, err error)
...
func (s *syncTargetClusterLister) List(selector labels.Selector) (ret []*edgev2alpha1.SyncTarget, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*edgev2alpha1.SyncTarget))
	})
	return ret, err
}
...
type SyncTargetLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*edgev2alpha1.SyncTarget, error)
...
func (s *syncTargetLister) List(selector labels.Selector) (ret []*edgev2alpha1.SyncTarget, err error) {
	err = kcpcache.ListAllByCluster(s.indexer, s.clusterName, selector, func(i interface{}) {
		ret = append(ret, i.(*edgev2alpha1.SyncTarget))
	})
	return ret, err
}

// Get retrieves the SyncTarget from the indexer for a given workspace and name.
func (s *syncTargetLister) Get(name string) (*edgev2alpha1.SyncTarget, error) {
	...
	if !exists {
		return nil, errors.NewNotFound(edgev2alpha1.Resource("SyncTarget"), name)
	}
	return obj.(*edgev2alpha1.SyncTarget), nil
}
...
// List lists all SyncTargets in the indexer for a workspace.
func (s *syncTargetScopedLister) List(selector labels.Selector) (ret []*edgev2alpha1.SyncTarget, err error) {
...
// Get retrieves the SyncTarget from the indexer for a given workspace and name.
func (s *syncTargetScopedLister) Get(name string) (*edgev2alpha1.SyncTarget, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
