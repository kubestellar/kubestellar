...
type SinglePlacementSliceClusterLister interface {
	...
	List(selector labels.Selector) (ret []*edgev2alpha1.SinglePlacementSlice, err error)
...
func (s *singlePlacementSliceClusterLister) List(selector labels.Selector) (ret []*edgev2alpha1.SinglePlacementSlice, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*edgev2alpha1.SinglePlacementSlice))
	})
	return ret, err
}
...
type SinglePlacementSliceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*edgev2alpha1.SinglePlacementSlice, error)
...
func (s *singlePlacementSliceLister) List(selector labels.Selector) (ret []*edgev2alpha1.SinglePlacementSlice, err error) {
	err = kcpcache.ListAllByCluster(s.indexer, s.clusterName, selector, func(i interface{}) {
		ret = append(ret, i.(*edgev2alpha1.SinglePlacementSlice))
	})
	return ret, err
}

// Get retrieves the SinglePlacementSlice from the indexer for a given workspace and name.
func (s *singlePlacementSliceLister) Get(name string) (*edgev2alpha1.SinglePlacementSlice, error) {
	...
	if !exists {
		return nil, errors.NewNotFound(edgev2alpha1.Resource("SinglePlacementSlice"), name)
	}
	return obj.(*edgev2alpha1.SinglePlacementSlice), nil
}
...
// List lists all SinglePlacementSlices in the indexer for a workspace.
func (s *singlePlacementSliceScopedLister) List(selector labels.Selector) (ret []*edgev2alpha1.SinglePlacementSlice, err error) {
...
// Get retrieves the SinglePlacementSlice from the indexer for a given workspace and name.
func (s *singlePlacementSliceScopedLister) Get(name string) (*edgev2alpha1.SinglePlacementSlice, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
