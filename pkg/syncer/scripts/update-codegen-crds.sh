...
cd "${REPO_ROOT}/pkg/syncer"
"../../${CONTROLLER_GEN}" \
    crd \
    rbac:roleName=manager-role \
    webhook \
    paths="./..." \
    output:crd:artifacts:config="${REPO_ROOT}"/pkg/syncer/config/crds

for CRD in "${REPO_ROOT}"/pkg/syncer/config/crds/*.yaml; do
    if [ -f "${CRD}-patch" ]; then
        echo "Applying ${CRD}"
...
"${REPO_ROOT}/${API_GEN}" --input-dir "${REPO_ROOT}"/pkg/syncer/config/crds --output-dir "${REPO_ROOT}"/pkg/syncer/config/exports
