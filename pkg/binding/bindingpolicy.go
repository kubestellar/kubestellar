...
func (c *Controller) syncBindingPolicy(ctx context.Context, bindingPolicyName string) error {
	...
	if !isBeingDeleted(bindingPolicy) {
		...
		// If at least one StatusCollector object is missing, ensure the Condition and requeue the BindingPolicy,
		// o.w. unset the Condition and proceed
		...
		policyWithProposedStatus.Status = v1alpha1.BindingPolicyStatus{
			ObservedGeneration: bindingPolicy.Generation,
			Conditions:         v1alpha1.UnsetCondition(bindingPolicy.Status.Conditions, v1alpha1.TypeStatusCollectorsAvailable),
		}
		_, err = c.bindingPolicyClient.UpdateStatus(ctx, policyWithProposedStatus, metav1.UpdateOptions{FieldManager: ControllerName})
		if err != nil {
			return fmt.Errorf("failed to update status for BindingPolicy %s: %w", bindingPolicyName, err)
