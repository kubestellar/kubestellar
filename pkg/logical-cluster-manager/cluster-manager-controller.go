...
func (c *Controller) processAdd(ctx context.Context, key any) error {
	...
	providerInfo, err := c.clusterclientset.EdgeV1alpha1().ClusterProviderInfods().Get(ctx, newClusterConfig.Spec.ProviderName, v1.GetOptions{})
	if err != nil {
...
func (c *Controller) processDelete(ctx context.Context, key any) error {
	...
	providerInfo, err := c.clusterclientset.EdgeV1alpha1().ClusterProviderInfods().Get(ctx, delClusterConfig.Spec.ProviderName, v1.GetOptions{})
	if err != nil {
		logger.Error(err, "failed to get ClusterProviderInfo.")
		return err
	}
