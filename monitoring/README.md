...
For OpenShift clusters add the flags `--env ocp` & `--hub-context <hub-context-name>` in the commad above. 
...
```bash
./configure-monitoring-wds.sh --host-cluster-context kind-kubeflex
```
...
```bash
./configure-monitoring-its.sh --host-cluster-context kind-kubeflex
```
