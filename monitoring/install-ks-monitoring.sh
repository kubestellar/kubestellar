...
while [ $# != 0 ]; do
    case "$1" in
        (-h|--help) echo "$0 usage: (--cluster-context (e.g., --cluster-context core-cluster (default value: kind-kubeflex)) | --monitoring-ns (e.g., --monitoring-ns monitoring (default value: ks-monitoring)) | --set (e.g., --set wec (default value: core)) | --env (e.g., --env ocp (default value: k8s)) | --hub-context (e.g., --hub-context kscore (default value: kind-kubeflex)) *"
                    exit;;
        (--cluster-context)
          if (( $# > 1 )); then
            ctx="$2"
...
case "$env" in
    (k8s|ocp) ;;
    (*) echo "$0: --env must be 'k8s' or 'ocp'" >&2
        exit 1;;
esac
...
if [ $opt == "core" ];then
  ...
  if [ $env == "k8s" ];then
       ...
       thanos_host=$(kubectl --context $hub_context -n $ns get route thanos-receive -o jsonpath="{.status.ingress[0].host}")
       export THANOS_ENDPOINT="https://$thanos_host"
       export MinIO_ENDPOINT=$(kubectl --context $hub_context -n $ns get route minio-api -o jsonpath="{.status.ingress[0].host}")
