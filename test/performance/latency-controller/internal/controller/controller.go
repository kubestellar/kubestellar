...
func (r *GenericLatencyCollectorReconciler) processGenericObject(ctx context.Context, obj *unstructured.Unstructured) {
	...
	if !exists {
		...
		shouldRemoveFromCache := r.processClusters(ctx, entry, key, true)

		if shouldRemoveFromCache || !hasStatusField {
			delete(r.cache, key)
	...
	if hasStatusField {
		oldStatusTime := entry.wdsObjectStatusTime
		newStatusTime := r.getGenericStatusTime(obj)

		if !newStatusTime.IsZero() && newStatusTime.After(oldStatusTime) {
			entry.wdsObjectStatusTime = newStatusTime

			shouldRemoveFromCache := r.processClusters(ctx, entry, key, false)
