...
1. Deploy the KS control plane hosting infra: 
    ...
    ARCH="x86_64"
    EC2_AMI="<aws-ami>"
    KS_RELEASE="{{ config.ks_latest_release }}"
    ```

    Then, deploy the core infrastructure which includes a VPC, security groups, EC2 instances, etc.
    ```bash
    cd test/scale-infra
    ./deploy_ks_cp_infra.sh --region us-east-2 --vpc-name $VPC --k8s-num-workers $NUM_WORKER_NODES --instances-type $EC2_INSTANCE_TYPE --aws-key-name $EC2_SSH_PUBLIC_KEY --arch $ARCH --ec2-image-id $EC2_AMI --ks-release $KS_RELEASE
    ```
...
2. Create WEC hosting instances:
    ...
    ```bash
    ./deploy_wec_infra.sh --region $REGION --vpc-name $VPC --wecs-hosting-instances $NUM_HOSTING_INSTANCES --instances-type $EC2_INSTANCE_TYPE --aws-key-name $EC2_SSH_PUBLIC_KEY --arch $ARCH --ec2-image-id $EC2_AMI
    ```
