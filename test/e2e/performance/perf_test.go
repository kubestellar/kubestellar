...
var _ = ginkgo.Describe("performance tests", func() {
	...
	ginkgo.Context("scalability", func() {
		ginkgo.It("create a bindingpolicy and a single deployment", func() {
			const numDeployments = 10
			sumBinding, sumManifestwork, sumDeploymentWDS, sumDeploymentWEC, sumTotal := 0, 0, 0, 0, 0

			for i := 0; i < numDeployments; i++ {
				util.CreateBindingPolicy(ctx, ksWdsClient, "performance",
				...
				start := time.Now()
				util.CreateDeployment(ctx, wdsClient, ns, fmt.Sprint(i),
					map[string]string{
						"app.kubernetes.io/name": "nginx",
					})
				util.ValidateNumDeployments(ctx, wec1Client, ns, 1)

				timeDeploymentWDS := int(util.GetDeploymentTime(ctx, wdsClient, ns, fmt.Sprint(i)).Sub(start).Seconds())
				...
				timeDeploymentWEC := int(util.GetDeploymentTime(ctx, wec1Client, ns, fmt.Sprint(i)).Sub(start).Seconds())
				...
				util.DeleteDeployment(ctx, wdsClient, ns, fmt.Sprint(i))
				util.ValidateNumDeployments(ctx, wec1Client, ns, 0)
				util.DeleteBindingPolicy(ctx, ksWdsClient, "performance")
			}
