...
kubestellar_version=0.26.0-alpha.4
...
if [ "$k8s_platform" == "kind" ]; then
    curl -s https://raw.githubusercontent.com/kubestellar/kubestellar/v${kubestellar_version}/hack/check_pre_req.sh \
        | bash -s -- --assert -V kflex ocm helm kubectl docker kind
else
    curl -s https://raw.githubusercontent.com/kubestellar/kubestellar/v${kubestellar_version}/hack/check_pre_req.sh \
        | bash -s -- --assert -V kflex ocm helm kubectl docker k3d
fi
...
context_clean_up() {
    ...
    while IFS= read -r ctx; do
        if [ "$ctx" == "cluster1" ]; then
            echo "Deleting cluster1 context..."
            kubectl config delete-context cluster1
        elif [ "$ctx" == "cluster2" ]; then
...
##########################################
# Wait for a given port to be free before continuing.
# $1 = port
# $2 = optional timeout (default: 60 seconds)
##########################################
wait_for_port_free() {
    local port="$1"
    local timeout="${2:-60}"
    ...
    if ! command_exists "lsof"; then
        echo "WARNING: 'lsof' not found. Skipping port check for port: $port."
        return 0
    fi

    echo "Checking if port $port is free..."
    while lsof -i :"$port" >/dev/null 2>&1; do
        echo "Port $port is in use; waiting 5 seconds..."
        sleep 5
        if [[ $(( $(date +%s) - start_time )) -ge $timeout ]]; then
            echo "ERROR: Port $port is still in use after $timeout seconds."
            exit 1
        fi
    done
...
for cluster in "${clusters[@]}"; do
    if [ "$k8s_platform" == "kind" ]; then
        ...
        kind create cluster --name "${cluster}" \
            >"${cluster_log_dir}/${cluster}.log" 2>&1 && touch "${cluster_log_dir}/${cluster}.success" &
    else
        # Create k3d clusters in parallel as well
        k3d cluster create --network k3d-kubeflex "${cluster}" \
            >"${cluster_log_dir}/${cluster}.log" 2>&1 && touch "${cluster_log_dir}/${cluster}.success" &
    fi
done

# Wait for all background tasks (both cluster1 and cluster2 creation)
wait

echo -e "Creating KubeFlex cluster with SSL Passthrough"
if [ "$k8s_platform" == "kind" ]; then
    # This is a synchronous script call
    curl -s \
         https://raw.githubusercontent.com/kubestellar/kubestellar/v${kubestellar_version}/scripts/create-kind-cluster-with-SSL-passthrough.sh \
         | bash -s -- --name kubeflex --nosetcontext
else
    # Ensure port 9443 is free before binding
    wait_for_port_free 9443 120
    # Also a synchronous command
...
for cluster in "${clusters[@]}"; do
    ...
    kubectl config rename-context "${k8s_platform}-${cluster}" "${cluster}" >/dev/null 2>&1 || true
done
if [ "$some_failed" = true ]; then
    exit 10
fi

# Double-check context renaming
for cluster in "${clusters[@]}"; do
...
kubectl --context $k8s_platform-kubeflex wait controlplane.tenancy.kflex.kubestellar.org/its1 \
    --for 'jsonpath={.status.postCreateHooks.its-with-clusteradm}=true' --timeout 24h
kubectl --context $k8s_platform-kubeflex wait -n its1-system job.batch/its-with-clusteradm \
    --for condition=Complete --timeout 24h
echo -e "\nWaiting for OCM hub cluster-info to be updated..."
kubectl --context $k8s_platform-kubeflex wait -n its1-system job.batch/update-cluster-info \
    --for condition=Complete --timeout 24h
...
for cluster in "${clusters[@]}"; do
   clusteradm --context its1 get token | grep '^clusteradm join' \
       ...
       | awk '{print $0 " --context '${cluster}' --singleton '${flags}'"}' \
       | sh
done

echo -e "\nWaiting for cluster1 and cluster2 CSRs to appear and approve them..."
checking_cluster cluster1
checking_cluster cluster2

echo ""
echo "Checking the new clusters are in the OCM inventory and labeling them..."
kubectl --context its1 get managedclusters
kubectl --context its1 label managedcluster cluster1 location-group=edge name=cluster1
kubectl --context its1 label managedcluster cluster2 location-group=edge name=cluster2

echo -e "\n\033[33m✔\033[0m Congratulations! Your KubeStellar demo environment is now ready to use."
