...
if [ $verbosity == 1 ]; then
    ...
    ./kcp/bin/kubectl-kcp workload edge-sync $syncer_name --syncer-image $img -o $syncTarget_name-syncer.yaml
else
    ...
    ./kcp/bin/kubectl-kcp workload edge-sync $syncer_name --syncer-image $img -o $syncTarget_name-syncer.yaml > /dev/null 2>&1
fi
