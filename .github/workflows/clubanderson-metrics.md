      ...
      - name: Fetch all GitHub data
        ...
        run: |
          ...
          gh api search/issues \
            -f q="org:kubestellar is:issue label:\"help wanted\" author:clubanderson created:>=${{ steps.dates.outputs.date_60 }}" \
          ...
          gh api search/issues \
            -f q="org:kubestellar is:pr commenter:clubanderson updated:>=${{ steps.dates.outputs.date_60 }}" \
            --jq '{total_count, items: [.items[] | {number, title, html_url, state}]}' \
            > /tmp/metrics-data/prs-commented.json
          
          # Search 3: Merged PRs authored
          gh api search/issues \
            -f q="org:kubestellar is:pr is:merged author:clubanderson merged:>=${{ steps.dates.outputs.date_60 }}" \
          ...
          gh api search/issues \
            -f q="org:kubestellar is:issue is:open repo:kubestellar/docs repo:kubestellar/ui repo:kubestellar/ui-plugins" \
          ...
          gh api search/issues \
            -f q="org:kubestellar is:pr is:open repo:kubestellar/docs repo:kubestellar/ui repo:kubestellar/ui-plugins" \
            --jq '{total_count, items: [.items[] | {number, title, html_url, repository_url, labels: [.labels[].name], created_at}]}' \
