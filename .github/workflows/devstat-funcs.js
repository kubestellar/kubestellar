...
function buildGitHubQuery({ username, repos, filters, qualifier, sinceDate, suffix = '', prefix = '' }) {
...
function GET_OPEN_PR_COUNT_SINCE(username, sinceDate, repos = DEFAULT_REPOS) {
  const query = buildGitHubQuery({
    username, repos, filters: "is:pr is:open", qualifier: "author", sinceDate
  });
...
function GET_MERGED_PR_COUNT_SINCE(username, sinceDate, repos = DEFAULT_REPOS) {
  const query = buildGitHubQuery({
    username, repos, filters: "is:pr is:merged", qualifier: "author", sinceDate
  });
  return callGitHubGraphQL(query);
}
...
function GET_ASSIGNED_ISSUE_COUNT(username, repos = DEFAULT_REPOS) {
  const query = buildGitHubQuery({
    username, repos, filters: "is:issue is:open", qualifier: "assignee"
  });
  return callGitHubGraphQL(query);
}

function GET_ASSIGNED_ISSUE_COUNT_SINCE(username, sinceDate, repos = DEFAULT_REPOS) {
  const query = buildGitHubQuery({
    username, repos, filters: "is:issue is:open", qualifier: "assignee", sinceDate
  });
  return callGitHubGraphQL(query);
}

function GET_ASSIGNED_ISSUE_COUNT_DYNAMIC(username, daysAgo, repos = DEFAULT_REPOS) {
  const sinceDate = getDateXDaysAgo(daysAgo);
...
function GET_HELP_WANTED_COUNT_SINCE(username, sinceDate, repos = DEFAULT_REPOS) {
  const query = buildGitHubQuery({
    username, repos, filters: 'is:issue label:\\"help wanted\\"', qualifier: "author", sinceDate
  });
  return callGitHubGraphQL(query);
}

function GET_HELP_WANTED_COUNT_DYNAMIC(username, daysAgo, repos = DEFAULT_REPOS) {
  const sinceDate = getDateXDaysAgo(daysAgo);
...
function GET_COMMENTED_MERGED_AND_OPEN_PR_COUNT_SINCE(username, sinceDate, repos = DEFAULT_REPOS) {
  const mergedQuery = buildGitHubQuery({
    username, repos, filters: "is:pr is:merged", qualifier: "commenter", sinceDate, suffix: "_merged"
  });
  const openQuery = buildGitHubQuery({
    username, repos, filters: "is:pr is:open", qualifier: "commenter", sinceDate, suffix: "_open"
  });
...
function GET_COMMENTED_MERGED_AND_OPEN_PR_COUNT_DYNAMIC(username, daysAgo, repos = DEFAULT_REPOS) {
  const sinceDate = getDateXDaysAgo(daysAgo);
  return GET_COMMENTED_MERGED_AND_OPEN_PR_COUNT_SINCE(username, sinceDate, repos);
}
