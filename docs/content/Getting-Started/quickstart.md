...
```shell
while [[ $(kubectl get pod commond -n commonstuff -o jsonpath='{.status.phase}') != "Running" ]]; do sleep 5; done;curl http://localhost:8094
```
...
```shell
while [[ $(kubectl get pod commond -n commonstuff -o jsonpath='{.status.phase}') != "Running" ]]; do sleep 5; done;curl http://localhost:8096
```
