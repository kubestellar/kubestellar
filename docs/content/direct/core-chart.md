...
This section shows a complete installation process with actual command outputs.
...
### Step 3: Create and Configure Namespace (Best Practice)

```bash
kubectl create namespace kubeflex-system
```

**Output:**
```
namespace/kubeflex-system created
```

**Apply Helm management annotations (recommended):**
```bash
kubectl annotate namespace kubeflex-system meta.helm.sh/release-name=ks-core --overwrite
kubectl annotate namespace kubeflex-system meta.helm.sh/release-namespace=kubeflex-system --overwrite
kubectl label namespace kubeflex-system app.kubernetes.io/managed-by=Helm --overwrite
```

**Output:**
```
namespace/kubeflex-system annotated
namespace/kubeflex-system annotated
namespace/kubeflex-system labeled
```

> **Note:** These annotations ensure proper cleanup when uninstalling the Helm chart and follow Kubernetes best practices for resource management.

### Step 4: Install KubeStellar Core Chart
...
```
NAME: ks-core
LAST DEPLOYED: Sun May 25 09:57:32 2025
NAMESPACE: kubeflex-system
STATUS: deployed
REVISION: 1
TEST SUITE: None
...
### Step 5: Verify Control Planes

```bash
kubectl get controlplane
```

**Initial Output (Control planes starting):**
```
NAME   SYNCED   READY   TYPE       AGE
its1   True     False   vcluster   2m43s
wds1   True     False   k8s        2m42s
```

**Final Output (Control planes ready):**
```
NAME   SYNCED   READY   TYPE       AGE
its1   True     True    vcluster   7m29s
wds1   True     True    k8s        7m28s
```

### Step 6: Setup Kubeconfig Contexts
...
> **Note:** The "No resources found in default namespace" output is **expected and normal behavior**. This indicates that:
> - The control plane contexts are working correctly
> - These are fresh control planes with no workloads deployed yet
> - The setup is ready for deploying applications and creating binding policies
...
kubestellar-report   Active   6m54s
...
open-cluster-management       Active   8m13s
open-cluster-management-hub   Active   7m6s
customization-properties      Active   6m27s
```
