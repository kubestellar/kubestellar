...
1. If you previously installed KS on K3D:
    ...
    ```
   If previously were running KS on Kind, clean that up with [the Kind cleanup script](../../../test/e2e/common/cleanup.sh).
...
1. Install kubestellar controller and OCM space:
   ...
   To specify passthru for K3D, edit the ingress placement controller and add `--enable-ssl-passthrough` to the list of args for the container
    ```shell
...
1. Create the Workload Execution Cluster `wec1` and register it
   Make sure `wec1` shares the same docker network as the hosting cluster `kubeflex`
    ```shell
    k3d cluster create -p "31080:80@loadbalancer"  --network k3d-kubeflex wec1
    kubectl config rename-context k3d-wec1 wec1
    ```
   [Paolo says] when we use kind, the name of the container is kubeflex-control-plane and that is what we use 
   ...
   Wait 60 seconds for the server to get going again, then register `wec1`:
    ```shell
   ...
   Wait for csr to be created:
    ...
    ```
    and then accept cluster and label it for the BindingPolicy:
    ```shell
...
statefulset `vcluster` in the `imbs1-system` namespace, both fully ready.
    ...
    ```shell
    kubectl --context k3d-kubeflex get deployments,statefulsets --all-namespaces
    ```
